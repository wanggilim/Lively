...
import com.quadcore.lively.twitter.controller.TwitterStatusController;
import com.quadcore.lively.twitter.model.TwitterStatus;
import com.quadcore.lively.twitter.util.TwitterHelper;
	...
	public static void main(String[] args) {
		...
		for (String nameToSearch : namesToSearch) {
			...
			try {
				...
				if (res != null) {
					for (Status statusFromTwitter : res) {
						...
						stmt = statusFromTwitter.getText();
			...
			if (statuses != null && statuses.size() > 0) {
				...
				// 추후 API 프로젝트(LivelyAPI) 통합시 이전될 예정
